...
export type LookupConfig = {
  /** URL segment, e.g. `product-categories`. Also the permission prefix. */
  resource: string
  /** Key the API uses for the collection in its list envelope. */
 ...
 *      permission prefix (`brands.viewAny`, `brands.create`, …), so it must
 *      match the API exactly. `collectionKey` is the key the API wraps the
 *      list in, which is snake_case where the URL is kebab-case.
...
const lookupConfigs: LookupConfig[] = [
  ...
  },
  {
    ...
    collectionKey: 'product_statuses',
  ...
  },
  {
    ...
    collectionKey: 'unit_of_measures',
