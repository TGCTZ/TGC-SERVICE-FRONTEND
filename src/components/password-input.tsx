 ...
 *
 * Being able to reveal what you typed reduces failed sign-ins, particularly on
 * phones. The toggle is a real `<button>` carrying an `sr-only` label that
 * tracks its state, so it is reachable and announced rather than being a
 * decorative icon.
 *
 * Accepts every input prop except `type`, which it owns.
 *
 * @param props.className - Merged onto the wrapper, not the input
 */
