...
/** A tab set. Controlled via `value`/`onValueChange`. */
function Tabs({
  className,
  ...props
...
/**
 * One tab. Its `value` selects the matching `TabsContent`.
 *
 * The selected tab is marked three ways at once - white `card` surface against
 * the `muted` track, a border, and foreground text at a heavier weight - because
 * any one of them alone is too quiet to read at a glance. The surface step in
 * particular cannot carry it: `background` is only about 3% off `muted` in light
 * mode, which is the same near-invisible gap `--table-header` exists to avoid.
 *
 * Unselected tabs are `muted-foreground` and brighten on hover, so the pair
 * reads as one selected and one available rather than two live options.
 */
