...
/**
 * Owns the sidebar's open/closed state and its keyboard shortcut.
 *
 * Persists to the `sidebar_state` cookie, which the layout reads before
 * first paint so a collapsed sidebar does not flash open on reload.
 *
 * Desktop and mobile are **separate states**: `open` for the docked sidebar,
 * `openMobile` for the sheet. Collapsing on desktop should not close the
 * overlay a phone user just opened, so read `isMobile` before deciding which
 * one you mean.
 *
 * Distinct from `context/layout-provider.tsx`, which holds the user's
 * *preferences* rather than this transient state.
 */
 ...
 * The sidebar shell.
 *
 * `collapsible` picks the collapse behaviour — `icon` leaves a rail,
 * `offcanvas` slides away entirely and takes its trigger with it, `none`
 * never collapses. Below `md` it always renders as a `Sheet` regardless.
 */
function Sidebar({
  side = 'left',
  variant = 'sidebar',
    ...
    return (
      <Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
        <SheetContent
          ...
          data-slot='sidebar'
          data-mobile='true'
          className='w-(--sidebar-width) bg-sidebar p-0 text-sidebar-foreground sm:w-(--sidebar-width) [&>button]:hidden'
          style={
            {
...
/** The thin strip along the sidebar's edge; clicking or dragging it toggles collapse. */
...
/** The main content area beside the sidebar. Sized so content reflows as the sidebar collapses. */
...
/** The top region, above the scrolling navigation. Does not scroll away. */
...
/** The bottom region, holding the user menu. Does not scroll away. */
...
/** A dividing rule between sidebar sections. */
...
/** The scrolling middle region holding the navigation groups. */
...
/** A titled section of navigation. */
...
/** A group's heading. Fades out in icon mode, where there is no room for it. */
...
/** A control in a group heading's trailing corner. */
...
/** The items belonging to a group. */
...
/**
 * A navigation link or button.
 *
 * Set `isActive` for the current route. Give it a `tooltip`: in icon mode
 * the label is hidden and the tooltip becomes the only way to tell items
 * apart.
 */
...
/** A secondary control on a menu row, such as a submenu toggle. */
...
/** A count or status pill on a menu row. Hidden in icon mode. */
...
/** A loading placeholder shaped like a menu row, for navigation awaiting permissions. */
...
/** A nested list under a menu item. Hidden in icon mode, where sub-items appear in a fly-out. */
...
/** A link inside a `SidebarMenuSub`. */
